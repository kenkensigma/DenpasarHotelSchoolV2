...
<head>
  ...
  @endphp
    ...
    <title>News - Denpasar Hotel School</title>
    <link rel="shortcut icon" type="education" href="img/logo_192509100957_ 1.png" />
    <link rel="stylesheet" href="{{ asset('css/news-after.css') }}">
...
<div class="container">
        <div class="main-content">
            <a href="{{ route('news') }}">
                <div class="backbtn"><i class='bx bxs-chevron-left'></i>{{ $translator->translate('Back') }}</div>
            </a>
            <img alt="{{ $news->caption }}" class="main-image" src="{{ url("storage/{$news->photo}") }}" width="800" height="400"/>
            ...
            <div class="news-meta">
                ...
                {{ $translator->translate($news->category) }}
               <i class="fas fa-eye" style="margin-left: 10px;"></i>
               {{ $news->hit ?? '0' }}
               <div class="btn-medsos">
                <!-- WhatsApp -->
                <a class="Whatsapp" href="https://api.whatsapp.com/send?text={{ urlencode($news->title . ' ' . url()->current()) }}" target="_blank">
                    <i class='bx bxl-whatsapp'></i>
                </a>
            
                <!-- Facebook -->
                <a class="Facebook" href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank">
                    <i class='bx bxl-facebook'></i>
                </a>
            
                <!-- Telegram -->
                <a class="Telegram" href="https://t.me/share/url?url={{ urlencode(url()->current()) }}&text={{ urlencode($news->title) }}" target="_blank">
                ...
                <!-- More Options -->
                <a id="more-options" class="see-more" href="javascript:void(0);">
            ...
            <div id="shareModal" class="modal">
                <div class="modal-content">
                    ...
                    <p>Copy link</p>
                    <input type="text" id="profileLink" value="{{ url()->current() }}" readonly>
            ...
            <div class="news-content">
                {!! $translator->translate($news->content) !!}
            </div>
        </div>
        <div class="sidebar-news">
<div class="devider">
    <span class="line-devider"></span>  
    <p>{{ $translator->translate('Recent News') }}</p>
    <span class="line-devider"></span>
</div>
    
            @foreach ($sideNews as $newsItem)
            @if ($newsItem->status == 1) {{-- Hanya tampilkan berita dengan status "Show" --}}
            <a href="{{ route('news.detail', $newsItem->id) }}">
            <div class="sidebar-item">
                <img alt="{{ $newsItem->title }}" height="100" src="{{ url("storage/{$newsItem->photo}") }}" width="100"/>
                <div class="sidebar-item-content">
                    <div class="sidebar-item-title">
                        <h3>{{  $translator->translate( Str::limit($newsItem->title, 50) ) }} </h3>
                        <p>{{ $translator->translate( Str::limit(strip_tags($newsItem->content), 80) ) }} </p>
                    </div>   
                </div>
            </div>
        </a>
            ...
            <div class="see-all-news">
                <a href="{{ route('news') }}">{{ $translator->translate('See all news ') }}<i class='bx bxs-chevron-right'></i></a>
            </div>
        </div>
    </div>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var btn = document.getElementById("more-options");
    ...
    // Pastikan modal selalu tertutup saat halaman dimuat
    modal.style.display = "none";

    // Event untuk membuka modal saat tombol diklik
    btn.onclick = function () {
        modal.style.display = "flex"; // Menggunakan flex agar modal tetap di tengah
    };
    ...
    // Event untuk menutup modal saat mengklik di luar area modal
...
// Fungsi untuk menyalin link profil
function copyProfileLink() {
    var copyText = document.getElementById("profileLink");

    if (!copyText) {
        console.error("Elemen input profileLink tidak ditemukan!");
        return;
    }

    copyText.select();
    copyText.setSelectionRange(0, 99999); // Untuk kompatibilitas di perangkat mobile
    document.execCommand("copy");
